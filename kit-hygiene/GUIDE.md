...
```bash
# Clone or copy kit-hygiene/ into your repo (or alongside it)
# From your repo root:
cp kit-hygiene/.env.example kit-hygiene/.env
```
...
Open `kit-hygiene/.env` in your editor. Here's what each variable does:
...
Output: JSON reports in `kit-hygiene/reports/` (one per scanner).
...
```bash
uv run kit-hygiene/scanners/find_silent_killers.py --scripts   # static only
uv run kit-hygiene/scanners/find_silent_killers.py             # full (with LLM)
```
...
```bash
kit-hygiene/run-registry-scan.sh              # full mode
kit-hygiene/run-registry-scan.sh --scripts    # static only
```
...
Each scanner writes two files to `kit-hygiene/reports/`:
...
- **Full reset**: `rm kit-hygiene/reports/*.json` then re-run.
...
| Scanner imports fail (`ModuleNotFoundError`) | Run `pip install -r kit-hygiene/requirements.txt` from repo root |
...
```bash
# Edit the list
echo "./src/problem_file.py" > kit-hygiene/scanners/kill_tries_list.txt

# Run the batch refactor
uv run kit-hygiene/scanners/kill_tries.py

# Apply approved refactors
uv run kit-hygiene/scanners/kill_tries_apply.py

# Verify
uv run kit-hygiene/scanners/kill_tries_post_check.py
```
...
```
AIB_FACTORY_ROOT       — repo root holding kit-hygiene/
