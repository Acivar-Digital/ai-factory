...
# Annot: your-repo-only (src2.* imports). Honour KIT_PATH / TARGET_REPO override.
...
async def run_test(verbose=False, chat_id_override=None):
    ...
    db.set_user_prefs(user_id, sifu_mode=0)
    ...
    # 2. Turn 1: Daily Forecast Ingestion
    logger.info("--- TURN 1: /daily ---")
    if not run_real_llm:
    ...
    # 3. Turn 2: Follow-up question (Intent QA)
    logger.info("--- TURN 2: Follow-up Ask (Intent & Date QA) ---")
    if not run_real_llm:
    ...
    # 4. Turn 3: Fact Injection
    logger.info("--- TURN 3: Fact Injection (Career Update) ---")
    if not run_real_llm:
    ...
    # 5. Turn 4: Verify persistence (Memory QA)
    logger.info("--- TURN 4: Memory Verification (Context QA) ---")
    ...
    # 5. Turn 4: Memory Recall
    logger.info("--- TURN 4: Memory Recall Ask ---")
    if not run_real_llm:
