...
# ruff: noqa: E402
"""
GOLD Standard E2E Test Runner for your-repo

Usage:
    # your-repo-only: TEST/GOLD/run.py not in kit download. See KIT_PATH-based run via 'uv run pytest examples'.              # Run all tests
    # your-repo-only: TEST/GOLD/run.py not in kit download. See KIT_PATH-based run via 'uv run pytest examples'. --test 01_start  # Run specific test
    # your-repo-only: TEST/GOLD/run.py not in kit download. See KIT_PATH-based run via 'uv run pytest examples'. --list       # List all tests
    # your-repo-only: TEST/GOLD/run.py not in kit download. See KIT_PATH-based run via 'uv run pytest examples'. --verbose    # Verbose output
"""
...
def start_server():
    ...
    server_process = subprocess.Popen(
        # [your-repo-only: TEST/GOLD/00_infra/test_start.py not in kit download]
        ["uv", "run", "python", "TEST/GOLD/00_infra/test_start.py", "--reset", "--skip-preflight"],  # [your-repo-only: not in kit download]
