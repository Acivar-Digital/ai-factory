...
A bare checkout of `kit-tests/` with **no API keys and no `.env`** runs only the
...
- `kit-tests/conftest.py` — `collect_ignore` lists every `0N_*` dir (not collected).
...
```
RuntimeError: KIT_LIVE=true but missing required env: KIT_PATH, KIT_BASE_URL, KIT_MODEL, KIT_API_KEY — set them in kit-tests/.env.
```
