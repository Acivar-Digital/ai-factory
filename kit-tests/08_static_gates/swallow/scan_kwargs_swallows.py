...
# Annot: src2 is your-repo-only; honour KIT_PATH override, else kit-relative.
_kit_path = os.getenv("KIT_PATH", "")
TARGET_DIR = os.path.join(_kit_path, "src2") if _kit_path else str(Path(__file__).resolve().parents[4] / "your-repo" / "src2")
...
def main():
    ...
    if not os.path.exists(TARGET_DIR):
        print(f"⚠️  Target directory '{TARGET_DIR}' does not exist — skipping (your-repo-only).")
        return 0
