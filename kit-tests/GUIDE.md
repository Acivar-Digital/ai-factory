...
> **Note:** You only need the `kit-tests/` subfolder. This kit is a standalone
...
2. **`kit-tests/conftest.py`** has a `collect_ignore` list that names every
...
- If `KIT_LIVE=true` and **any** of `KIT_PATH`, `KIT_BASE_URL`, `KIT_MODEL`,
  ...
  ```
  RuntimeError: KIT_LIVE=true but missing required env: KIT_PATH, KIT_API_KEY — set them in kit-tests/.env.
  ```
