...
def load_manual_whitelist() -> set[str]:
    """Load whitelisted symbol names from admin/whitelist.txt if it exists."""
    whitelist = set()
    whitelist_file = Path("admin/whitelist.txt")
    if whitelist_file.exists():
        try:
            ...
            print(f"Warning: Could not read admin/whitelist.txt: {e}", file=sys.stderr)
    return whitelist
