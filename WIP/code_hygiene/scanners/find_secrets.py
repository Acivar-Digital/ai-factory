...
class SecretsExtractor(ast.NodeVisitor):
    def __init__(self, file_path: Path):
        ...
        # Match common secret patterns: sk-..., bearer tokens, long hex, etc.
    ...
    def visit_Assign(self, node: ast.Assign):
        # Look for target names like api_key, token, secret, password, auth
        is_secret_target = False
        target_name = ""
        for target in node.targets:
            if isinstance(target, ast.Name):
                target_name = target.id
                if any(k in target_name.lower() for k in ["api_key", "token", "secret", "password", "auth_key", "jwt"]):
                    is_secret_target = True

        if is_secret_target and isinstance(node.value, ast.Constant) and isinstance(node.value.value, str):
            val = node.value.value
            # Exclude short placeholders and empty values
            if len(val) > 6 and not val.startswith("{{") and not val.endswith("}}"):
                preview = val[:4] + "..." + val[-4:] if len(val) > 8 else val
    ...
    def visit_Constant(self, node: ast.Constant):
        if isinstance(node.value, str):
            # Check string literal directly for patterns
            match = self.secret_regex.search(node.value)
            if match:
                preview = node.value[:4] + "..." + node.value[-4:] if len(node.value) > 8 else node.value
                self.candidates.append(
                    SecretCandidate(
                        name="string_literal",
                        file_path=str(self.file_path),
...
def generate_markdown_report(report: AuditReport, md_path: Path):
    with open(md_path, "w", encoding="utf-8") as out:
        out.write("# 🕵️ Hardcoded Secrets Audit Report\n\n")
        ...
        if not report.audit_results:
            ...
            for f, list_results in sorted(by_file.items()):
                ...
                for item in sorted(list_results, key=lambda x: x.line):
                    status_emoji = "🛑" if item.status == "HARDCODED_SECRET" else "✅"
...
def main():
    ...
    output_dir = Path("admin/code_hygiene/reports")
