...
def main():
    ...
    # Filter files: must be Python files in src2/ or admin/ and not in exceptions
    files_to_scan = []
    for f in changed_files:
        path = Path(f)
        if path.suffix == ".py" and (f.startswith("src2/") or f.startswith("admin/")):
    ...
    reports_dir = Path("admin/code_hygiene/reports")
