...
Primary config: edit auditor.yaml in this directory, then run:
    ...
    uv run python -m admin.auditor_template.audit

CLI flags override auditor.yaml when provided:

    uv run python -m admin.auditor_template.audit --since 2026-07-01
    uv run python -m admin.auditor_template.audit --commit abc123 --commit def456
    uv run python -m admin.auditor_template.audit --last 5
    uv run python -m admin.auditor_template.audit --tier security

Tiers: security | logic | performance | telemetry | all (default: all)
Output: admin/auditor_template/reports/<timestamp>_<scope>.md
"""
...
mcp_codebase = MCPToolset(
    StdioTransport(MCP_PYTHON, args=[MCP_SCRIPT]),
)
...
AUDIT STRATEGY:
1. This is a pure text completion audit. You have NO tools registered. Do not attempt to call any tools (including gather_codebase_context).
2. You are provided with:
   - The git diff of the changes.
   - A pre-computed codebase dependency scan showing all active caller files and lines for modified or deleted functions/classes.
3. Use the pre-computed caller reference scan to determine if any remaining code relies on deleted logic, imports, or objects.
...
- Never invent findings. Only report what you can prove from the gathered evidence.
- NEVER use CHANGELOG.md, README.md, or any documentation file as evidence that a bug was fixed. Only source code (.py, .sql, .yaml) counts.
"""
...
def resolve_diff(cfg: dict, args: argparse.Namespace) -> tuple[str, str]:
    ...
    # Load paths filter from YAML config
    paths = cfg.get("paths", [])
    ...
    if commits:
        if len(commits) == 1:
            ...
            diff = _run(["git", "diff", f"{commits[0]}^", commits[0]] + path_args)
        else:
            scope = f"commits {commits[0][:8]}..{commits[-1][:8]}"
            diff = _run(["git", "diff", commits[0], commits[-1]] + path_args)
        return scope, diff
    ...
    if since:
        ...
        diff = _run(["git", "diff", f"{oldest}^", newest] + path_args)
        return scope, diff

    # 3. Last N commits
    last = args.last or cfg.get("last")
    if last is not None:
        diff = _run(["git", "diff", f"HEAD~{last}", "HEAD"] + path_args)
        scope = f"last {last} commits"
        return scope, diff

    # 4. Default: Audit uncommitted changes in working directory (staged and unstaged)
    diff = _run(["git", "diff", "HEAD"] + path_args)
    if not diff.strip():
        # Fallback if there are absolutely no changes in working directory: last 1 commit
        print("   No uncommitted changes found. Defaulting to audit the last commit.")
        diff = _run(["git", "diff", "HEAD~1", "HEAD"] + path_args)
        scope = "last 1 commit (fallback)"
    else:
        scope = "uncommitted working directory changes"
    return scope, diff

# ---------------------------------------------------------------------------
# LOCAL DEPENDENCY GATHERER (DETERMINISTIC & FREE)
# ---------------------------------------------------------------------------
# ---------------------------------------------------------------------------
# LOCAL DEPENDENCY GATHERER (DETERMINISTIC & FREE)
# ---------------------------------------------------------------------------
def local_gather_context(diff_text: str) -> str:
    """
    Parses the git diff deterministically to identify all deleted/modified functions,
    classes, and imports, and scans the codebase for any remaining references.
    Returns a formatted Markdown report of STALE references only.
    """
    import os
    import re
    from pathlib import Path

    # 1. Identify deleted/modified functions, classes, and imports (ignore local vars)
    deleted_symbols = set()

    # Match: - def function_name(...), - class ClassName...
    def_pattern = re.compile(r"^-\s*(?:def|class)\s+([a-zA-Z_][a-zA-Z0-9_]*)")
    # Match: - from module import symbol, symbol2
    import_pattern = re.compile(r"^-\s*from\s+[\w\.]+\s+import\s+(.+)")
    # Match: - import symbol
    direct_import_pattern = re.compile(r"^-\s*import\s+([a-zA-Z_][a-zA-Z0-9_]*)")
    ...
    for line in diff_text.splitlines():
        ...
        if line.startswith("-"):
            ...
            if m_def:
                deleted_symbols.add(m_def.group(1))
                continue

            m_imp = import_pattern.match(line)
            if m_imp:
                parts = [p.strip().split(" as ")[0] for p in m_imp.group(1).split(",")]
                for p in parts:
                    if p.isidentifier():
                        deleted_symbols.add(p)
                continue

            m_dir = direct_import_pattern.match(line)
            if m_dir:
                deleted_symbols.add(m_dir.group(1))

    # Clean up symbols list (ignore short names or python keywords)
    banned_keywords = {
        "self", "cls", "None", "True", "False", "import", "from", "return", "pass", "def", "class",
        "try", "except", "raise", "async", "await", "with", "as", "for", "in", "if", "else", "elif",
        "and", "or", "not", "is", "lambda", "global", "nonlocal", "break", "continue", "yield", "assert"
    }
    deleted_symbols = {s for s in deleted_symbols if len(s) > 2 and s not in banned_keywords}

    if not deleted_symbols:
        return ""

    # 2. Grep references across the codebase
    project_root = Path(__file__).resolve().parents[2]
    exclude_dirs = {"__pycache__", ".git", ".venv", "node_modules", ".agent", ".gemini", "admin", "scratch"}

    reference_map = {}
    print("\n   🔍 Scanning codebase for stale callers of cleaned symbols...")
    print("   " + "─" * 56)

    for symbol in sorted(deleted_symbols):
        reference_map[symbol] = []
    ...
    # 3. Format only the STALE symbols to show in prompt and terminal
    report_lines = []
    report_lines.append("# Stale Callers & Imports Identified in codebase\n")

    stale_count = 0
    for symbol in sorted(deleted_symbols):
        refs = reference_map[symbol]
        if not refs:
            # Clean symbols are printed to CLI but left out of prompt to avoid bloat
            print(f"   ✅ Symbol: `{symbol}` -> Clean (0 references).")
        else:
            stale_count += 1
            print(f"   🚨 Symbol: `{symbol}` -> STALE! {len(refs)} references found.")
            report_lines.append(f"### Stale References for: `{symbol}`")
            for r in refs[:20]:  # Cap at 20 references to keep prompt small
                report_lines.append(f"  - `{r['file']}:{r['line']}`: {r['text']}")
            report_lines.append("")

    print("   " + "─" * 56)

    # Save complete report to disk
    reports_dir = project_root / "admin" / "auditor_template" / "reports"
    reports_dir.mkdir(parents=True, exist_ok=True)
    (reports_dir / "gathered_context.md").write_text("\n".join(report_lines), encoding="utf-8")
    print("   📝 Saved gathered context report to: admin/auditor_template/reports/gathered_context.md\n")

    if stale_count == 0:
        return "All deleted functions and classes are clean. No remaining callers found in the codebase."
...
async def run_audit(
    ...
    - Runs a local deterministic dependency check first to collect caller context.
    - Injects only stale references directly into the tier prompt context.
    - Each tier gets a smart-truncated diff (change lines prioritised).
    - Live streaming via event_stream_handler shows thinking in real time.
    ...
    """
    # 1. Run local gathering first (Deterministic, local, and free!)
    stale_report = local_gather_context(diff)

    # 2. Initialize the main Auditor Agent (exposing read-only codebase tools for fallback checks)
    agent = Agent(
    ...
    async def no_mcp_context():
        yield

    async with no_mcp_context():
        for tier in tiers:
            ...
            prompt = (
                ...
                + f"STALE DEPENDENCY SCAN (Remaining references to modified/deleted symbols):\n"
                f"```markdown\n{stale_report}\n```\n\n"
