...
mcp_codebase = MCPToolset(
    ...
    # Tool filtering via process_tool_call is available in 2.x if needed
)
...
# ---------------------------------------------------------------------------
# AUDIT PLAN MODELS
# Phase 0: planner produces this before the main audit agent runs.
# ---------------------------------------------------------------------------
class AuditHypothesis(BaseModel):
    check_id: str = Field(description="Short ID e.g. 'C1', 'C2'")
    what: str = Field(description="Specific claim to verify in one sentence")
    tools_to_use: list[str] = Field(
        description="MCP tools needed e.g. ['grep_codebase', 'read_file']"
    )
    max_calls: int = Field(description="Max tool calls budgeted for this check (1-8)", ge=1, le=8)


class AuditPlan(BaseModel):
    tier: str = Field(description="Tier name being audited")
    hypotheses: list[AuditHypothesis] = Field(
        description="Ordered list: Critical risks first, then Medium, then Low"
    )
    total_budget: int = Field(description="Sum of max_calls across all hypotheses")
    approach: str = Field(description="One sentence rationale for the audit strategy")
    ...
    *,
    ...
    async for event in events:
        if isinstance(event, FunctionToolCallEvent):
            ...
            content = str(event.content)  # 2.x: .result.content → .content
            preview = content[:320]
...
# ---------------------------------------------------------------------------
# PHASE 0: AUDIT PLANNER
# Lightweight no-tool agent that reads the diff and produces a structured
# AuditPlan before the main audit agent fires any tool calls.
# ---------------------------------------------------------------------------
async def _generate_audit_plan(
    tier: str,
    diff_summary: str,
    notes: str,
) -> AuditPlan:
    """Run a lightweight no-tool planner and return a structured AuditPlan."""
    plan_agent = Agent(
        CONTROL_SHEET.auditor_model,
        output_type=AuditPlan,
        instructions=(
            "You are a senior audit planner. Given a git diff and tier focus, "
            "produce a structured AuditPlan: an ordered list of specific hypotheses "
            "to verify, each with the MCP tools to use and a call budget. "
            "Be concrete — name exact functions, variables, or file paths from the diff. "
            "Total budget across all hypotheses must not exceed 20 tool calls. "
            "Order: data loss / broken inserts first, then security, then type errors, then style."
        ),
    )
    prompt = (
        f"TIER: {tier.upper()}\n\n"
        ...
        + (f"Auditor Notes:\n{notes}\n\n" if notes else "")
        + f"Git Diff (first 1500 lines):\n```diff\n{diff_summary[:6000]}\n```\n\n"
        "Produce an AuditPlan with 4-8 specific hypotheses derived from the diff above."
    )
    result = await plan_agent.run(prompt, usage_limits=UsageLimits(request_limit=8))
    return result.output


def _print_audit_plan(plan: AuditPlan) -> None:
    """Print the audit plan clearly before execution begins."""
    print(f"\n  {'=' * 58}")
    print(f"  📋 AUDIT PLAN -- {plan.tier.upper()} TIER")
    print(f"  Strategy : {plan.approach}")
    print(f"  Budget   : {plan.total_budget} tool calls across {len(plan.hypotheses)} checks")
    print(f"  {'─' * 58}")
    for h in plan.hypotheses:
        tools_str = ", ".join(h.tools_to_use) if h.tools_to_use else "read_file"
        print(f"  {h.check_id:3s}  {h.what}")
        print(f"       Tools: {tools_str}  |  Budget: {h.max_calls} calls")
    print(f"  {'=' * 58}\n")
...
async def run_audit(
    ...
    async with mcp_codebase:
        for tier in tiers:
            ...
            # ── Phase 0: Plan ─────────────────────────────────────────
            print(f"  🗺  Planning audit for {tier.upper()} ...")
            try:
                plan = await _generate_audit_plan(tier, diff, notes)
                _print_audit_plan(plan)
                plan_context = (
                    "\n\nAUDIT PLAN (follow this order):\n"
                    + "\n".join(
                        f"  {h.check_id}. {h.what} (budget: {h.max_calls} calls)"
                        for h in plan.hypotheses
                    )
                    + "\n"
                )
            except Exception as plan_exc:
                print(f"  ⚠️  Planner failed ({plan_exc}) — proceeding without plan.")
                plan_context = ""
            ...
            try:
                ...
                import traceback
                print(f"\n  ❌ [{tier.upper()}] CRASHED — {type(exc).__name__}: {exc}")
                print("  Full traceback:")
                traceback.print_exc()
                print("\n  AUDIT ABORTED. Fix the error above and re-run.")
                raise
